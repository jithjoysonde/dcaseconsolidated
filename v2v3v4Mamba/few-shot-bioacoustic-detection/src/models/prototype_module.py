...
class PrototypeModule(LightningModule):
    ...
    def on_test_epoch_end(self):
        ...
        for k in self.onset_offset.keys():
            ...
            res_v2 = self.post_process_v2(alpha=k)  # Enhanced post-processing
            ...
            if res_v2["fmeasure"] > best_f_measure:
                best_result = res_v2
                best_f_measure = res_v2["fmeasure"]
    ...
    def post_process_v2(self, dataset="VAL", alpha=0.9):
        """Enhanced post-processing with NMS and adaptive duration filtering."""
        from src.utils.evaluation import evaluate
        from src.utils.post_proc_v2 import post_processing_v2
        ...
        for nms_iou in [0.2, 0.3, 0.4, 0.5]:
            team_name = "PostProcV2_nms%.1f" % nms_iou
            print(f"Testing NMS IoU threshold: {nms_iou}")
            new_evaluation_file = "%s/Eval_%s_postproc_v2_nms%.1f.csv" % (
                alpha,
                dataset,
                nms_iou,
            )
            post_processing_v2(
                ...
                use_duration_filter=False,  # Disabled - was too aggressive
                duration_n_std=2.5,  # More lenient if re-enabled
                use_merge=False,
                min_duration=0.03,  # Lower minimum
            )
            (
                overall_scores,
                individual_file_result,
        ...
        print("******************POST-PROC V2 BEST RESULT*****************")
        for k in best_result[1].keys():
            print(k, best_result[1][k])
        print(best_result[0], f"NMS IoU: {best_result[2]}")

        self.log_result(
            best_result[0],
            best_result[3],
            best_result[4],
            name="postproc_v2_nms%.2f" % (best_result[2]),
        )
        return best_result[0]
